...
Все преобразующие методы (`lower`, `upper`, `trim`, `append`, ...) **не изменяют исходный объект**,
а возвращают новый экземпляр. Оригинал всегда остаётся нетронутым:

```php
$s = Str::of('HELLO');
$s->lower();              // результат выброшен — НЕПРАВИЛЬНО, $s всё ещё 'HELLO'

$s2 = $s->lower();        // правильно
echo $s;                  // 'HELLO'
echo $s2;                 // 'hello'
```

Чейнинг работает как раньше — каждый вызов получает новый экземпляр и передаёт его дальше:

```php
$result = Str::of($input)->trim()->lower()->slug()->toString();
```

Единственное исключение: `limit()` и `words()` возвращают `$this` (тот же объект),
если строка уже помещается в лимит — значение при этом не меняется.
