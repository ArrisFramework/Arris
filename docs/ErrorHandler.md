...
- **по умолчанию** — по времени браузера: с **22:00 до 8:00** включается ночь (`dark`),
  иначе день (`light`). 8:00 уже считается днём, 22:00 — уже ночью;
- **вручную** — кнопка-переключатель в правом верхнем углу шапки дампа показывает **цель**
  переключения: ☾ (включить ночь) при светлой теме, ☀ (включить день) при тёмной;
- выбор запоминается в `localStorage` (`arris-theme`) и имеет приоритет над дефолтом;
  применяется при следующей ошибке без «вспышки» (тема ставится инлайн-скриптом в `<head>`
  до отрисовки).
