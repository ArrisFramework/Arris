...
## Динамические методы для полной перестройки РТ-индекса. 
...
## Статические методы работы с РТ-индексом через пакет `foolz/sphinxql-query-builder`
...
```
use function ArrisHelpers\array_map_to_integer as array_map_to_integer;
``` 
...
```
init(string $sphinx_connection_host, string $sphinx_connection_port, $options = [])
