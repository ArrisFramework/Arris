...
class App implements AppInterface
{
     ...
     *
     * readonly защищает ССЫЛКУ, а не объект: сам Dot мутабелен (add/set/delete работают),
     * но свойство нельзя перепривязать к другому Dot (в наследниках или извне).
     * Фиксирует инвариант контейнера: у инстанса App ровно один репозиторий опций,
     * собранный в конструкторе, и далее он меняется только через публичный API (add/set/...).
     * У синглтона с жизненным циклом, равным жизни приложения, "замена репозитория целиком"
     * не предусмотрена — readonly это документирует и защищает.
     *
     * @var Dot
     */
    ...
    /**
     * Репозиторий сервисов (DI).
     *
     * Аналогично $options: readonly гарантирует, что репозиторий создается один раз
     * при инициализации инстанса (в конструкторе) и не может быть пересоздан/обнулен
     * после этого. Сервисы добавляются через addService()/getService(), а не заменой объекта.
     *
     * @var Dot
     */
    ...
    /**
     * Конфиг инстанса (класс из пакета karelwintersky/arris.config).
     *
     * Также readonly: ссылка на объект конфига не переустанавливается,
     * изменения идут только через setConfig()/toConfig()/addConfig()/replaceConfig().
     *
     * @var AppConfig
     */
     ...
     *
     * Почему array|Dot, а не array|AppConfig:
     * - семантически на входе ДАННЫЕ (пары ключ-значение) для слияния,
     *   а не "другой конфиг-объект". Принять AppConfig значило бы извлекать
     *   из него ->all() и лить обратно — лишний indirection, семантика все
     *   равно сводится к массиву;
     * - AppConfig::add() в пакете arris.config сам принимает array, конфиги
     *   сливаются только через массивы (arrayMergeRecursiveReplace);
     * - Dot|array — минимально достаточный контракт: приходят данные из
     *   массива или из любого Dot-репозитория (App::add(), dot() и т.п.).
     *   Сигнатура унаследована от времен, когда конфиг держался в Dot.
     *
